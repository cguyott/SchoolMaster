...
BEGIN
    ...
    BEGIN TRY
        SELECT @PersonId = [PersonId] FROM [dbo].[Instructor] WHERE [Id] = @InstructorId

        IF (@PersonId > 0)
        BEGIN
            BEGIN TRANSACTION
                UPDATE [dbo].[Instructor] SET [Department] = @Department, [Position] = @Position WHERE [Id] = @InstructorId;
