CREATE PROCEDURE [dbo].[CreateInstructor]
(
    @Prefix NVARCHAR(6), 
...
BEGIN
    ...
    BEGIN TRY
        SELECT @Count = COUNT(*) FROM Person WHERE [Login] = @Login;
        ...
        BEGIN
            BEGIN TRANSACTION
                ...
                SELECT @PersonId = Id FROM [dbo].[Person] WHERE [Login] = @Login;
                ...
                SELECT @InstructorId = [Id] FROM [dbo].[Instructor] WHERE [PersonId] = @PersonId;

                INSERT INTO [dbo].[PersonRoleMap]
                (
                    [PersonId],
                    [RoleId]
                )
        ...
        ELSE
        BEGIN
            SET @Results = 1;  -- Indicates that this instructor already exists.
        END
    END TRY
    BEGIN CATCH
        IF (@@TRANCOUNT > 0)
